...
export default defineComponent({
  ...
  setup() {
   ...
   supabase.channel('public:realtime_chat')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'realtime_chat' }, (payload) => {
                  fetchMsg()
          if (!isExpand.value) {
    ...
    const fetchMsg = async () => {
      const { data } = await supabase.from<Message>("realtime_chat").select("*, realtime_user(name, color)")
